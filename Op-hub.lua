...
task.spawn(function()
	while true do
		local currentlyTouching = isTouchingSpawn()

		if currentlyTouching and not touching and AntiRagdollEnabled then
			touching = true
			disableRagdollRemotes()
			print("Started touching SpawnArea")
		elseif not currentlyTouching and touching and not AntiRagdollEnabled then
			touching = false
			EnableRagdollRemotes()
			print("stopped touching SpawnArea")
		end
